...
class TruePeopleGrabber(PageGrabber):
    ...
    def true_try(self, lookup, information):
        ...
        if lookup == "name":  # Make the URL for name lookup, set email to False
            agerange = raw_input(
                "[{}?{}] {}Please enter an age range:{} [ex: 18-100{}]: ".format(
                    bc.CRED, bc.CEND, bc.CRED, bc.CYLW, bc.CEND))
            citystatezip = raw_input(
                "[{}?{}] {}Please enter a city,state,or zip?{} [ex:(AL|Alabama|12345){}]: ".format(
                    bc.CRED, bc.CEND, bc.CRED, bc.CYLW, bc.CEND))
        ...
        if self.check_for_captcha() == True:  # Check responce for sign of captcha
            ...
            self.iscomplete = raw_input(
                ...
                "] " +
                bc.CYLW +
                "Have you completed the CAPTCHA? " +
                bc.CEND)
            """if str(self.iscomplete).lower() in ['no',False,0]:
                ...
                pass"""
        try:
            for xnotfound in self.soup.findAll(
                    'div', {'class', 'row pl-1 record-count'}):
        ...
        try:
            deep = self.soup.find_all(
                'a', {
                    'class': [
                        'btn', 'btn-success', 'btn-lg', 'detail-link', 'shadow-form']})
            age = "Unknown"
            name = "Unknown"
            aklist = "Unknown"
            lives = "Unknown"
