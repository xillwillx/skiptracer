...
class AdvanceBackgroundGrabber(PageGrabber):
    ...
    def abc_try(self, lookup, information):
        ...
        if lookup == "email":  # Make the URL for email lookup, set email True
            if str(information).split('@')[1]:
                self.url = "https://www.advancedbackgroundchecks.com/emails/{}".format(
                    b64.b64encode(str(information)))
                email = True
        if lookup == "name":  # Make the URL for name lookup, set email to False
            if str(information).split(' ')[1]:
                age = raw_input(
                    "[{}?{}] {}Whats the target's suspected age?{} [ex: 40{}]: ".format(
                        bc.CRED, bc.CEND, bc.CRED, bc.CYLW, bc.CEND))
                loc = raw_input(
                    "[{}?{}] {}Whats the target's area of residency?{} [ex: MO/11123/Chicago{}]: ".format(
                        bc.CRED, bc.CEND, bc.CRED, bc.CYLW, bc.CEND))
                ...
                for xcheck in checkres:
                    if xcheck.text in [
                        ...
                        print("  [" + bc.CRED + "X" + bc.CEND + "] " +
                              bc.CYLW + "No results were found.\n" + bc.CEND)
                        return
            # .contents  # Scrape for JSON within DOM
        ...
        try:
            ...
            # Format data for JSON load
            script_htmla = str(script_htmla).strip()
        ...
        for person in person_list:  # Iterate entries and store their values, return results in different outputs, STDOUT, Dict()
            ...
            try:
                ...
                self.source2 = self.get_source(
                    self.url2)  # request 2nd level url
                ...
                print("  [" + bc.CGRN + "+" + bc.CEND + "] " +
                      bc.CRED + "Name: " + bc.CEND + str(person.get("name")))
                ...
                if len(script_html2) <= 1:
                    ...
                    if len(person_list2) > 1:
                        try:
                            print(
                                "  [" + bc.CGRN + "+" + bc.CEND + "] " + bc.CRED + "Phone: " + bc.CEND)
                        ...
                        try:
                            print(
                                "  [" + bc.CGRN + "+" + bc.CEND + "] " + bc.CRED + "Email: " + bc.CEND)
                            ...
                            print(
                                "   [" + bc.CGRN + "=" + bc.CEND + "] " + bc.CRED + "Addr: " + bc.CEND)
                            pass
