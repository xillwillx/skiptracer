    ...
    def truth_try(self, lookup, information):
        ...
        if lookup == "name":  # Make the URL for name lookup, set email to False
            citystatezip = raw_input(
                "[{}?{}] {}Please enter a city,state,or zip?{} [ex:(AL=Alabama|CO=Colorado){}]: ".format(
                    bc.CRED, bc.CEND, bc.CRED, bc.CYLW, bc.CEND))
            gender = raw_input(
                "[{}?{}] {}Please enter the targets biological sex?{} [ex:(M|F){}]: ".format(
                    bc.CRED, bc.CEND, bc.CRED, bc.CYLW, bc.CEND))
            age = raw_input(
                "[{}?{}] {}Is the person older than 30?{} [ex:(Y|n){}]: ".format(
                    bc.CRED, bc.CEND, bc.CRED, bc.CYLW, bc.CEND))
