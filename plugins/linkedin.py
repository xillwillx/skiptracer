...
class LinkedInGrabber(
        PageGrabber):  # LinkedIN.com sales scraper for email lookups
    def get_info(self, email):  # Requires AUTH, login and request AUTHENTICATED pages from linkedin
        ...
        if not login_information['session_key']:
            ...
            if login_information['session_password'] == '':
                ...
                print()
                savecreds = raw_input(
                    "[{}?{}] {}Would you like to save credentials now? {}(Y/n){}]: ".format(
                ...
                if str(savecreds).lower() in ['y', 'yes']:
                    try:
                        ...
                        print(
                            ("Failed to write fbinfo to file: %s") %
                            failedtowrite)
        try:
